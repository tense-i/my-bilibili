...
func main() {
	...
	fmt.Println("========== 测试专用数据库结构体 ==========")

	testAVIDs := []int64{100001, 100011, 100015}
	for _, avid := range testAVIDs {
		var video videoInfoDB
		err := db.QueryRowCtx(context.Background(), &video, query, avid)
		if err != nil {
			log.Printf("❌ AVID %d 查询失败: %v\n", avid, err)
			continue
		}
		...
		fmt.Printf("  PlayHive: %d (应该是 150000/250000/220000)\n", video.PlayHive)
		fmt.Printf("  LikesHive: %d (应该是 8000/15000/13000)\n", video.LikesHive)
		fmt.Printf("  PlayMonth: %d (应该是 50000/90000/85000)\n", video.PlayMonth)
		fmt.Printf("  LikesMonth: %d (应该是 2500/4500/4200)\n", video.LikesMonth)
	}
